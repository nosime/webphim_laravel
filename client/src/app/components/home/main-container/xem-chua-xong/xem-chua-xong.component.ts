...
@Component({
  selector: 'app-xem-chua-xong',
  standalone: true,
  imports: [RouterLink, NgFor,NgIf, NgClass, CarouselModule],
  templateUrl: './xem-chua-xong.component.html',
  styleUrl: './xem-chua-xong.component.css'
})

export class XemChuaXongComponent implements OnInit {
  movies_hs: Movie[] = [];
  currentPage = 0;
  totalPages = 6;
  loading = false;
  ...
  loadMovies() {
    ...
    this.movieService.getRandomMoviesPageLimit(4,48)
      .pipe(
      ...
      .subscribe({
        next: (response) => {
          if (response?.data?.length) {
            this.movies_hs = response.data.slice(0, 48);
          }
        },
        error: (err) => {
